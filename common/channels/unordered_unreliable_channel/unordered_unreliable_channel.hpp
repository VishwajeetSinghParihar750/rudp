...
namespace unordered_unreliable_channel_config
{
...
class channel_buffer
{
    ...
    uint32_t current_to_read_pos = 0;
...
public:
    ...
    ssize_t read_data(char *obuf, const uint32_t &sz)
    {
        ...
        auto &pkt = data.front();
        uint32_t off = rudp_protocol_packet::CHANNEL_HEADER_OFFSET + unordered_unreliable_channel_config::HEADER_SIZE;
        uint32_t packet_size = pkt->get_length() - off;
        uint32_t bytes_pending = packet_size - current_to_read_pos;

        uint32_t toread = std::min(bytes_pending, sz);
        memcpy(obuf, pkt->get_buffer() + off + current_to_read_pos, toread);

        if (bytes_pending > toread)
            current_to_read_pos += toread;
        else
        {
            data.pop();
            current_to_read_pos = 0;
        }
    ...
    bool put_rudp_protocol_packet(std::unique_ptr<rudp_protocol_packet> pkt)
    {
        ...
        uint32_t packet_size = pkt->get_length() - (rudp_protocol_packet::CHANNEL_HEADER_OFFSET)-unordered_unreliable_channel_config::HEADER_SIZE;
    ...
    std::unique_ptr<rudp_protocol_packet> get_rudp_protocol_packet()
    {
        ...
        uint32_t packet_size = ret->get_length() - (rudp_protocol_packet::CHANNEL_HEADER_OFFSET)-unordered_unreliable_channel_config::HEADER_SIZE;
...
public:
    explicit send_window(uint32_t size = unordered_unreliable_channel_config::DEFAULT_BUFFER_SIZE)
        : buffer(size) {}
    ...
    bool receive_bytes(const char *ibuf, const uint32_t &sz)
    {
        ...
        size_t off = (rudp_protocol_packet::CHANNEL_HEADER_OFFSET) + unordered_unreliable_channel_config::HEADER_SIZE;
        uint32_t len = off + sz;
...
public:
    explicit receive_window(uint32_t size = unordered_unreliable_channel_config::DEFAULT_BUFFER_SIZE)
        : buffer(size) {}
