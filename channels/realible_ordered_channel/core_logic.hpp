...
inline uint64_t get_random_uint64_t() { return 1000; } // Placeholder
...
public:
    ...
    void incoming_segment(sequence_number start_seq, sequence_number end_seq, sequence_number &rcv_nxt) {
        if (start_seq < rcv_nxt) start_seq = rcv_nxt;
        ...
        for (auto key : to_erase) sack_blocks.erase(key);
