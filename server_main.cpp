...
int main() {
    ...
    // Pre-register a range of channels
    for (int i = 1; i <= 1000; i++) {
        server->add_channel(i, channel_type::RELIABLE_ORDERED_CHANNEL);
    }
        ...
        while (true) {
            ...
            if (nread > 0) {
                ...
                if (nread < 10) {
                    ...
                    std::cout << "[DEBUG] Short message on ch" << ch_id << 
                             " from cl" << cl_id << ": " << msg << std::endl;
                }
            } else if (nread < 0) {
